...
public class FindController {
    ...
    @PostMapping("/writing")
    public ResponseEntity<Void> postWriting(@RequestBody FindWritingRequest request) {
        findWritingService.postWritingExecute(request);
        return new ResponseEntity<>(HttpStatus.CREATED);
    }

    @PatchMapping("/writing/{id}")
    public ResponseEntity<Void> patchWriting(@PathVariable Long id, @RequestBody ModifyWritingRequest request){
